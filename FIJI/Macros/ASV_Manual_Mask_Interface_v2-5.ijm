...
function im2stack(t_skip, input, output) {
    ...
    for (g = 0; g < groups.length; g++) {
        groupName = groups[g];
        groupFiles = newArray(0);
        
        // Collect files for this group
        for (i = 0; i < fileList.length; i++) {
            fileName = fileList[i];
            if (startsWith(fileName, groupName) && endsWith(fileName.toLowerCase(), ".tif")) {
                frameNumber = parseInt(substring(fileName, lastIndexOf(fileName, "_") + 2, lastIndexOf(fileName, ".")));
               	print(parseInt(substring(fileName, lastIndexOf(fileName, "_") + 2, lastIndexOf(fileName, "."))));
                print("Frame:"+frameNumber);
                if (frameNumber % t_skip == 1) {
                    groupFiles = Array.concat(groupFiles, fileName);
        ...
        // Sort group files
        Array.sort(groupFiles);
        print("GroupFiles length: " + groupFiles.length);
        
        // Create stack
        if (groupFiles.length > 0) {
        	print("Opening");
            File.openSequence(input, " filter="+groupName+" step=4");
