...
sort_stage_pos_folders(basename_list, path_output, stage_pos_maps, image_type, ms_choice)
    ...
    for thread_count in range(1, parallels + 1):
        ...
        if __name__ == "__main__":
            thread_handle = threading.Thread(target=run_threads, args=(
                sys.path[0] + '/multithread_processes/thread' + str(f"{thread_count:02}") + '.py', input_location,))
            thread_handle.start()
