...
class WellPlateInterface:
    ...
    def create_widgets(self):
        ...
        ttk.Button(self.master, text="Unassign Wells", command=self.unassign_wells).grid(row=1, column=3, padx=5,
                                                                                         pady=5)
        ttk.Button(self.master, text="Finish Assignment", command=self.end_selection).grid(row=1, column=4, padx=5,
                                                                                           pady=5)
        ttk.Button(self.master, text="Undo All", command=self.undo_all_assignments).grid(row=1, column=5, padx=5,
                                                                                         pady=5)
    ...
    def end_selection(self):
        # Check if all conditions have been assigned
        ...
        if assigned_wells_count == len(self.allowed_wells):  # All wells assigned
            print(assigned_wells_count, " wells are assigned. Closing interface.")
            root.destroy() # Terminate the main loop and close the interface
        else:
    ...
    def undo_all_assignments(self):
        # Clear all assignments and reset buttons
        for well in list(self.df['Well']):
...
stage_pos_map = {
    'tissue_bi': {1: 'Drift', 2: 'A01', 3: 'A04', 4: 'A05', 5: 'A07', 6: 'A08', 7: 'A09', 8: 'A10', 9: 'A11', 10: 'A12',
                  ...
                  20: 'C01', 21: 'C03', 22: 'C08', 23: 'C09', 24: 'C10', 25: 'C11', 26: 'C12', 27: 'D12', 28: 'H11'}
}
